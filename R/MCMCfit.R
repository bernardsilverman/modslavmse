...
#' 
#' Use the \code{MCMCpack} routine \code{MCMCpoisson} to fit a multiple systems estimation model. 
#'
#'
...
MCMCfit <-
function (zdata, priorprec=1, mainprec=1e-4, sigthresh=2,totalpopest=F,runzerothresh=NULL,...) 
{
#
#  given an incidence matrix of the Rcapture form, carry out a MCMC using the MCMCpoisson prior
#   Here priorprec=0 corresponds to an improper prior over (-inf, inf)
#   If priorprec is positive it is the reciprocal of the prior variance of the two factor interactions
#    priorprec0 is the reciprocal of the prior variance of the main effects in that case
#   If priorprec is negative then two-factor interactions are not fitted
#
#   If sigthresh > 0, then the analysis is repeated dropping out any effects whose posterior mean/SD does not exceed sigthresh
#
#   If totalpopest=T then output the MCMC of the dark figure plus the original data observed
#
nd2=dim(zdata)[2]
#  
#
#  set up model for MCMC poisson with interactions, distinguishing between priorprec=0 and priorprec > 0 
#   because of a bug in the MCMCpoisson program, if priorprec>0 all the diagonal elements of B0 have to be nonzero, so mainprec is used.
#
...
BB0=mainprec
if (priorprec < 0) form = makeform(zfull,NULL,F)
if (priorprec > 0) {form = makeform(zfull,NULL,T)
...
if (priorprec==0) { 
		...
		zcfd = removeemptyoverlaps(zfull)
            ...
            if (!is.null(rempairs)) nrem=length(rempairs)/2 else nrem=0
...
#
#  run MCMC poisson, unless runzerothresh is supplied, in which case that is the result of a previous run with sigthresh=0 and totalpopest=F
#   so should be read in
#
#
if (is.null(runzerothresh)) zMCMC = MCMCpoisson(formula= form, data=zfull, B0=BB0, beta.start=start1, ...) else zMCMC=runzerothresh
#
# if sigthresh> 0 then run again, dropping out any non-significant effects, thresholding at sigthresh
#    but if sigthresh is zero or no effects are below threshold, there is no refinement
#
...
if (totalpopest) zMCMC[,1]= nobserved + exp(zMCMC[,1])
return(zMCMC)
