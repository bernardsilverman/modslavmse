...
#' Put data into correct format for the Madigan and York method
#' @export
createarraydata= function(zdat) {
  m = dim(zdat)[2] - 1
  zarr = array(0, rep(2,m))
  indmult = 2^(0:(m-1))
  indices = 1 + as.matrix(zdat)[, 1:m] %*% indmult
  zarr[indices] = zdat[, m+1]
 return(zarr)
